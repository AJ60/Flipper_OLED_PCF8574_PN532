...
def apply_button_hover(btn, bg_active, fg_active, bg_idle, fg_idle):
    btn.bind("<Enter>", lambda e: btn.config(background=bg_active, foreground=fg_active))
...
class CustomMessagebox(tk.Toplevel):
    def __init__(self, parent, title, message, box_type="info", choices=None):
        ...
        # Outer border
        outer_frame = tk.Frame(self, bg=CONTAINER_COLOR, relief='solid', borderwidth=1, highlightthickness=1, highlightbackground=ACCENT_COLOR)
        outer_frame.pack(fill="both", expand=True)
        
        # Title bar
        title_lbl = tk.Label(outer_frame, text=title.upper(), font=("Segoe UI", 10, "bold"), fg=ACCENT_COLOR, bg=CONTAINER_COLOR)
        title_lbl.pack(fill="x", pady=(10, 5))
        
        # Message Text
        msg_lbl = tk.Label(outer_frame, text=message, font=("Segoe UI", 10), fg=TEXT_COLOR, bg=CONTAINER_COLOR, justify="center", wraplength=380)
        ...
        for choice in self.choices:
            btn = tk.Button(btn_frame, text=choice, font=("Segoe UI", 9, "bold"),
                            bg=BG_COLOR, fg=TEXT_COLOR, activebackground=ACCENT_COLOR, activeforeground=BG_COLOR,
                            relief='solid', borderwidth=1, highlightthickness=1, highlightbackground="#1b2a47",
                            padx=15, pady=5, command=lambda c=choice: self.set_result(c))
...
class DIYFlasherApp:
    def __init__(self, root):
        ...
        try:
            ...
            ctypes.windll.dwmapi.DwmSetWindowAttribute(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE, ctypes.byref(use_dark_mode), ctypes.sizeof(use_dark_mode))
            DWMWA_SYSTEMBACKDROP_TYPE = 38
            backdrop_type = ctypes.c_int(3) # Acrylic
            ctypes.windll.dwmapi.DwmSetWindowAttribute(hwnd, DWMWA_SYSTEMBACKDROP_TYPE, ctypes.byref(backdrop_type), ctypes.sizeof(backdrop_type))
        ...
        self.style.configure("TCombobox", fieldbackground=BG_COLOR, background=CONTAINER_COLOR, foreground=TEXT_COLOR,
                        darkcolor="#1b2a47", lightcolor="#1b2a47", bordercolor="#1b2a47", arrowcolor=ACCENT_COLOR)
    ...
    def create_widgets(self):
        ...
        title_lbl = tk.Label(header, text="🐬 DIY FLIPPER ZERO FLASHER", font=("Segoe UI", 16, "bold"), fg=ACCENT_COLOR, bg=BG_COLOR)
        title_lbl.pack(side="left")
        
        self.status_lbl = tk.Label(header, text="🔌 Checking device status...", font=("Segoe UI", 10, "bold"), fg=TEXT_MUTED, bg=BG_COLOR)
        ...
        self.tab_container = tk.Frame(notebook_frame, bg=CONTAINER_COLOR, relief='solid', borderwidth=1, highlightthickness=1, highlightbackground="#1b2a47")
        ...
        self.tab_btns = []
        btn1 = tk.Button(tab_buttons, text="GITHUB UPDATES", font=("Segoe UI", 9, "bold"), command=lambda: show_tab(0),
                         bg=BG_COLOR, fg=TEXT_COLOR, relief='solid', borderwidth=1, highlightthickness=1, highlightbackground="#1b2a47", padx=15, pady=5)
        btn1.pack(side="left", padx=(0, 5))
        self.tab_btns.append(btn1)
        apply_button_hover(btn1, ACCENT_COLOR, BG_COLOR, BG_COLOR, TEXT_COLOR)
        
        btn2 = tk.Button(tab_buttons, text="LOCAL FILE", font=("Segoe UI", 9, "bold"), command=lambda: show_tab(1),
                         bg=BG_COLOR, fg=TEXT_COLOR, relief='solid', borderwidth=1, highlightthickness=1, highlightbackground="#1b2a47", padx=15, pady=5)
        btn2.pack(side="left", padx=5)
        self.tab_btns.append(btn2)
        apply_button_hover(btn2, ACCENT_COLOR, BG_COLOR, BG_COLOR, TEXT_COLOR)
        
        btn3 = tk.Button(tab_buttons, text="OTP PROVISIONING", font=("Segoe UI", 9, "bold"), command=lambda: show_tab(2),
                         bg=BG_COLOR, fg=TEXT_COLOR, relief='solid', borderwidth=1, highlightthickness=1, highlightbackground="#1b2a47", padx=15, pady=5)
        ...
        log_lbl = tk.Label(log_frame, text="LOG CONSOLE OUTPUT", font=("Segoe UI", 9, "bold"), fg=TEXT_MUTED, bg=BG_COLOR)
        log_lbl.pack(anchor="w")
        
        self.log_txt = tk.Text(log_frame, height=8, bg=LOG_BG, fg=TEXT_COLOR, font=("Consolas", 9), relief="solid", borderwidth=1, highlightbackground="#1b2a47")
        ...
        self.style.configure("TProgressbar", thickness=10, troughcolor=BG_COLOR, background=ACCENT_COLOR)
    ...
    def setup_github_tab(self):
        tk.Label(self.tab1, text="SELECT FIRMWARE VERSION FROM GITHUB", font=("Segoe UI", 10, "bold"), fg=TEXT_COLOR, bg=CONTAINER_COLOR).pack(anchor="w", pady=(0, 5))
        ...
        tk.Label(self.tab1, text="RELEASE DETAILS", font=("Segoe UI", 9, "bold"), fg=TEXT_MUTED, bg=CONTAINER_COLOR).pack(anchor="w", pady=(10, 2))
        
        self.release_info_txt = tk.Text(self.tab1, height=10, bg=BG_COLOR, fg=TEXT_MUTED, font=("Segoe UI", 9), wrap="word", relief="solid", borderwidth=1, highlightbackground="#1b2a47")
        self.release_info_txt.pack(fill="both", expand=True, pady=5)
        
        self.gh_action_btn = tk.Button(self.tab1, text="DOWNLOAD & INSTALL", font=("Segoe UI", 10, "bold"),
                                       bg=ACCENT_COLOR, fg=BG_COLOR, activebackground=BUTTON_ACTIVE, activeforeground=BG_COLOR,
                                       relief="flat", pady=8, command=self.on_github_action)
        self.gh_action_btn.pack(fill="x", pady=(10, 0))
        apply_button_hover(self.gh_action_btn, BUTTON_ACTIVE, BG_COLOR, ACCENT_COLOR, BG_COLOR)

    # Tab 2: Local File Setup
    def setup_local_tab(self):
        tk.Label(self.tab2, text="FLASH LOCAL FIRMWARE OR UPDATE PACKAGE", font=("Segoe UI", 10, "bold"), fg=TEXT_COLOR, bg=CONTAINER_COLOR).pack(anchor="w", pady=(0, 5))
        
        browse_frame = tk.Frame(self.tab2, bg=CONTAINER_COLOR)
        browse_frame.pack(fill="x", pady=10)
        
        self.file_entry = tk.Entry(browse_frame, bg=BG_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 10), relief="solid", borderwidth=1, highlightbackground="#1b2a47")
        self.file_entry.pack(side="left", fill="x", expand=True, ipady=4, padx=(0, 5))
        
        browse_btn = tk.Button(browse_frame, text="BROWSE...", font=("Segoe UI", 9, "bold"), bg=BG_COLOR, fg=TEXT_COLOR, command=self.browse_local_file,
                               relief="solid", borderwidth=1, highlightthickness=1, highlightbackground="#1b2a47", padx=15, pady=4)
        ...
        self.local_file_details = tk.Label(info_frame, text="Please select a file:\n\n• .tgz files are update packages (flashed in Serial/COM mode)\n• .dfu/.bin files are full images (flashed in DFU Recovery mode)", 
                                           font=("Segoe UI", 9), fg=TEXT_MUTED, bg=BG_COLOR, relief="solid", borderwidth=1, justify="left", anchor="nw", padx=15, pady=15)
        self.local_file_details.pack(fill="both", expand=True)
        
        self.local_action_btn = tk.Button(self.tab2, text="INSTALL / FLASH LOCAL FILE", font=("Segoe UI", 10, "bold"),
                                          bg=ACCENT_COLOR, fg=BG_COLOR, activebackground=BUTTON_ACTIVE, activeforeground=BG_COLOR,
                                          relief="flat", pady=8, state="disabled", command=self.on_local_action)
        self.local_action_btn.pack(fill="x", pady=(10, 0))
        apply_button_hover(self.local_action_btn, BUTTON_ACTIVE, BG_COLOR, ACCENT_COLOR, BG_COLOR)

    # Tab 3: OTP Setup
    def setup_otp_tab(self):
        tk.Label(self.tab3, text="GENERATE & PROVISION HARDWARE OTP PROFILE", font=("Segoe UI", 10, "bold"), fg=TEXT_COLOR, bg=CONTAINER_COLOR).pack(anchor="w", pady=(0, 10))
        ...
        fields = [
            ...
            ("Board Version:", "BoardVersion", "Select Board revision number (e.g. 12)", "12"),
            ("Display Type:", "DisplayType", "MGG = SSD1306 OLED (DIY), ERC = standard LCD", ["MGG", "ERC"]),
            ("Body Color:", "BodyColor", "Color used in animations", ["Black", "White", "Transparent"]),
            ("Sub-GHz Region:", "Region", "Radio frequency band locks", ["Europe", "USA", "Japan", "World"])
        ]
        ...
        for idx, (label_text, var_name, tooltip_txt, default_val) in enumerate(fields):
            ...
            lbl = tk.Label(row_frame, text=label_text, font=("Segoe UI", 10), fg=TEXT_MUTED, bg=CONTAINER_COLOR, width=15, anchor="w")
            lbl.pack(side="left")
            ...
            if isinstance(default_val, list):
                ...
                cb = ttk.Combobox(row_frame, textvariable=var, values=default_val, state="readonly", font=("Segoe UI", 10))
                cb.pack(side="left", fill="x", expand=True)
                self.otp_vars[var_name] = var
            else:
                # Entry
                var = tk.StringVar(value=default_val)
                ent = tk.Entry(row_frame, textvariable=var, bg=BG_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 10), relief="solid", borderwidth=1, highlightbackground="#1b2a47")
        ...
        save_btn = tk.Button(actions_frame, text="1. SAVE .BIN FILE", font=("Segoe UI", 9, "bold"), bg=BG_COLOR, fg=TEXT_COLOR, command=self.save_otp_bin,
                             relief="solid", borderwidth=1, highlightthickness=1, highlightbackground="#1b2a47", pady=8)
        save_btn.pack(side="left", fill="x", expand=True, padx=(0, 5))
        apply_button_hover(save_btn, ACCENT_COLOR, BG_COLOR, BG_COLOR, TEXT_COLOR)
        
        self.otp_flash_btn = tk.Button(actions_frame, text="2. FLASH OTP (DFU)", font=("Segoe UI", 9, "bold"), bg=ACCENT_COLOR, fg=BG_COLOR, command=self.flash_otp_dfu,
                                       relief="flat", pady=8, state="disabled")
        self.otp_flash_btn.pack(side="left", fill="x", expand=True, padx=5)
        apply_button_hover(self.otp_flash_btn, BUTTON_ACTIVE, BG_COLOR, ACCENT_COLOR, BG_COLOR)
        
        self.otp_read_btn = tk.Button(actions_frame, text="3. READ OTP (DFU)", font=("Segoe UI", 9, "bold"), bg=BG_COLOR, fg=TEXT_COLOR, command=self.read_otp_dfu,
                                      relief="solid", borderwidth=1, highlightthickness=1, highlightbackground="#1b2a47", pady=8, state="disabled")
        self.otp_read_btn.pack(side="left", fill="x", expand=True, padx=(5, 0))
        apply_button_hover(self.otp_read_btn, ACCENT_COLOR, BG_COLOR, BG_COLOR, TEXT_COLOR)
    ...
    def check_connection_loop(self):
        def check():
            while True:
                ...
                if self.cli_path:
                    try:
                        res = subprocess.run([self.cli_path, "-l", "usb"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, creationflags=subprocess.CREATE_NO_WINDOW, timeout=1.5)
                ...
                if not dfu_connected:
                    ...
                    try:
                        ...
                        res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, creationflags=subprocess.CREATE_NO_WINDOW, timeout=1.5)
                ...
                if dfu_connected:
                    ...
                    if len(flippers) == 0:
                        ...
                        for p in all_ports:
                            ...
                            if "stm" in desc or "virtual com" in desc or "flipper" in desc:
                                flippers.append(p)
                                break
    ...
    def update_connection_status(self, mode, port):
        ...
        if mode == "DFU Recovery Mode":
            self.status_lbl.config(text="🟢 STATUS: DFU RECOVERY MODE (BOOT0)", fg="#00d2ff")
    ...
    def update_action_buttons(self):
        ...
        if not self.selected_file_path:
            self.local_action_btn.config(state="disabled", text="INSTALL / FLASH LOCAL FILE")
            return
            
        fn = self.selected_file_path.lower()
        if fn.endswith(".tgz"):
            if self.connected_device_mode == "Serial Mode":
                self.local_action_btn.config(state="normal", text="INSTALL OTA PACKAGE (SERIAL)")
            else:
                self.local_action_btn.config(state="disabled", text="TGZ REQUIRES SERIAL MODE")
        elif fn.endswith(".dfu") or fn.endswith(".bin"):
            if self.connected_device_mode == "DFU Recovery Mode":
                self.local_action_btn.config(state="normal", text="FLASH FIRMWARE (DFU)")
            else:
                self.local_action_btn.config(state="disabled", text="DFU/BIN REQUIRES DFU MODE")
        else:
            self.local_action_btn.config(state="disabled", text="UNSUPPORTED FILE FORMAT")
    ...
    def fetch_releases_async(self):
        def fetch():
            ...
            req = urllib.request.Request(url, headers={"User-Agent": "DIY-Flipper-Flasher"})
            try:
    ...
    def on_github_action(self):
        ...
        target_extension = ".tgz" if self.connected_device_mode == "Serial Mode" else ".dfu"
        if self.connected_device_mode == "Disconnected":
            CustomMessagebox.show_error(self.root, "Device Disconnected", "Please connect your DIY Flipper Zero in either Serial or DFU Recovery mode first.")
            return
            
        matching_assets = [a for a in assets if a["name"].lower().endswith(target_extension)]
        ...
        if not matching_assets:
            CustomMessagebox.show_error(self.root, "No Matching Asset", f"Could not find a suitable {target_extension} file in this GitHub release.")
            return
        ...
        if self.connected_device_mode == "DFU Recovery Mode":
            if not CustomMessagebox.ask_yes_no(self.root, "Confirm Recovery Flash", "Are you sure you want to write this firmware? This will overwrite the entire memory of the device."):
                return
        ...
        def download_and_install_thread():
            ...
            try:
                ...
                self.root.after(0, lambda: self.gh_action_btn.config(state="normal", text="DOWNLOAD & INSTALL"))
    ...
    def run_dfu_flash(self, filepath):
        if self.connected_device_mode != "DFU Recovery Mode":
            CustomMessagebox.show_error(self.root, "Not in DFU Mode", "The device must be in DFU mode to write .dfu or .bin firmware.")
            return
        ...
        if not tool_path:
            CustomMessagebox.show_error(self.root, "No Flash Utility Found", "Could not locate STM32_Programmer_CLI or dfu-util on your PC.\nPlease install STM32CubeProgrammer.")
            return
    ...
    def on_flash_completed(self, success):
        ...
        if success:
            ...
            CustomMessagebox.show_info(self.root, "Flash Success", "Firmware flashed successfully!\n\nYour DIY Flipper Zero will now reboot.")
        else:
            self.log("Flash failed! Please check DFU connections and logs.", "ERROR")
            CustomMessagebox.show_error(self.root, "Flash Failed", "Flashing process encountered errors. Check logs for details.")
    ...
    def run_serial_ota_install(self, tgz_path):
        if self.connected_device_mode != "Serial Mode" or not self.connected_port:
            CustomMessagebox.show_error(self.root, "Not in Serial Mode", "The device must be running and connected to a COM port to install .tgz updates.")
            return
        ...
        def ota_thread():
            ...
            try:
                ...
                if not update_fuf_path:
                    self.log("Could not find update.fuf manifest inside package!", "ERROR")
                ...
                self.log(f"Uploading files to Flipper SD card via {self.connected_port}...")
                ...
                with FlipperStorage(self.connected_port) as storage:
                    ...
                    storage.send_and_wait_eol(f"update install {flipper_update_path}/update.fuf\r")
                    res = storage.read.until(storage.CLI_EOL)
                    if b"Verifying" in res:
                        res = storage.read.until(storage.CLI_EOL)
                        if res.startswith(b"OK"):
                            success = True
                            self.log("OTA Command acknowledged! Flipper will now reboot and install.")
                        else:
                            self.log(f"OTA Install error: {res.decode('ascii')}", "ERROR")
                    else:
                        self.log(f"Unexpected response to update command: {res.decode('ascii')}", "ERROR")
    ...
    def on_ota_completed(self, success):
        ...
        if success:
            ...
            CustomMessagebox.show_info(self.root, "OTA Update Started", "Update files copied successfully!\n\nYour DIY Flipper Zero has rebooted to perform the update.")
        else:
            self.log("OTA Update failed. Check connections and logs.", "ERROR")
            CustomMessagebox.show_error(self.root, "Update Failed", "OTA Update failed. See log output for details.")
    ...
    def save_otp_bin(self):
        ...
        try:
            ...
            with open(filepath, "wb") as f:
                f.write(data)
            CustomMessagebox.show_info(self.root, "Saved", f"OTP Profile binary saved successfully to:\n{filepath}")
        except Exception as e:
            CustomMessagebox.show_error(self.root, "Save Failed", f"Failed to generate and save OTP: {str(e)}")
    ...
    def flash_otp_dfu(self):
        if self.connected_device_mode != "DFU Recovery Mode":
            CustomMessagebox.show_error(self.root, "Not in DFU Mode", "The device must be in DFU mode to flash OTP.")
            return
            
        if not CustomMessagebox.ask_yes_no(self.root, "Irreversible Write Warning", 
        ...
        try:
            ...
            if not tool_path:
                CustomMessagebox.show_error(self.root, "CLI Not Found", "STM32_Programmer_CLI is required for OTP flashing (dfu-util is not supported for OTP).")
                return
            ...
            def flash_thread():
                ...
                try:
                    ...
                    cmd = [tool_path, "-c", "port=usb1", "-d", otp_file, "0x1FFF7000", "-v"]
                    self.log(f"Running command: {' '.join(cmd)}")
                    proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
                    for line in proc.stdout:
                        clean_line = clean_cli_log(line)
            ...
            CustomMessagebox.show_error(self.root, "Error", f"Could not prepare OTP data: {str(e)}")
    ...
    def on_otp_flashed(self, success):
        ...
        if success:
            ...
            CustomMessagebox.show_info(self.root, "Success!", f"Successfully flashed OTP profile to address 0x1FFF7000!\n\nYou can now reboot the device.")
        else:
            self.log("OTP Flash failed. Check connections and logs.", "ERROR")
            CustomMessagebox.show_error(self.root, "OTP Flash Failed", "Flashing OTP failed. See logs for details.")

    def read_otp_dfu(self):
        if self.connected_device_mode != "DFU Recovery Mode":
            CustomMessagebox.show_error(self.root, "Not in DFU Mode", "The device must be in DFU mode to read OTP.")
            return
        ...
        tool_path = self.cli_path
        if not tool_path:
            CustomMessagebox.show_error(self.root, "CLI Not Found", "STM32_Programmer_CLI is required for OTP reading.")
            return
        ...
        def read_thread():
            ...
            try:
                ...
                cmd = [tool_path, "-c", "port=usb1", "-r32", "0x1FFF7000", "32", read_file]
                self.log(f"Running command: {' '.join(cmd)}")
                proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
                for line in proc.stdout:
                    clean_line = clean_cli_log(line)
    ...
    def on_otp_read(self, success, filepath):
        ...
        if not success:
            ...
            CustomMessagebox.show_error(self.root, "Read Failed", "Could not read OTP from device. Check logs.")
            return
        ...
        try:
            ...
            # Parse OTP structure
            import struct
            magic, ver, res1, timestamp, board_ver, target, body, connect, disp, res2, res3, color, region, res4, res5, name_bytes = struct.unpack(
                "<HBBIIBBBBBBHBBH8s", data[:32]
            )
            ...
            self.log(f"Timestamp: {timestamp} ({time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(timestamp))})")
            self.log(f"Board Version: {board_ver}")
            self.log(f"Board Target: {target}")
            self.log(f"Display Type: {'MGG (SSD1306 OLED)' if disp == 2 else 'ERC (Standard LCD)'} (Value: {disp})")
            ...
            self.log(f"Sub-GHz Region: {region_map.get(region, 'Unknown')} (Value: {region})")
            self.log("=" * 50)
            ...
            CustomMessagebox.show_info(self.root, "OTP Data Loaded", f"Successfully read OTP for '{name}' and loaded it into the form.")
            
        except Exception as e:
            self.log(f"Failed to parse OTP data: {str(e)}", "ERROR")
            CustomMessagebox.show_error(self.root, "Parsing Error", f"Failed to parse read OTP data: {str(e)}")
