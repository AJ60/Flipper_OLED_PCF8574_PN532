...
#define INFRARED_TX_CCMR_HIGH \
    (TIM_CCMR1_OC1PE | LL_TIM_OCMODE_PWM2)
#define INFRARED_TX_CCMR_LOW \
    (TIM_CCMR1_OC1PE | LL_TIM_OCMODE_FORCED_INACTIVE)
...
/* Static DMA buffers: no malloc/free during IR TX */
static uint16_t infrared_tx_data_0[INFRARED_TIM_TX_DMA_BUFFER_SIZE];
static uint16_t infrared_tx_data_1[INFRARED_TIM_TX_DMA_BUFFER_SIZE];

static uint8_t infrared_tx_polarity_0[INFRARED_TIM_TX_DMA_BUFFER_SIZE + INFRARED_POLARITY_SHIFT];
static uint8_t infrared_tx_polarity_1[INFRARED_TIM_TX_DMA_BUFFER_SIZE + INFRARED_POLARITY_SHIFT];
...
static void furi_hal_infrared_tx_fill_buffer_last(uint8_t buf_num) {
    ...
    furi_check(buffer->data);
    furi_check(buffer->polarity);

    buffer->data[0] = 0;
    buffer->polarity[0] = INFRARED_TX_CCMR_LOW;
    buffer->data[1] = 0;
    buffer->polarity[1] = INFRARED_TX_CCMR_LOW;
    buffer->size = 2;
    buffer->last_packet_end = true;
    buffer->packet_end = true;
}
    ...
    for(*size = 0; (*size < INFRARED_TIM_TX_DMA_BUFFER_SIZE) &&
        ...
        if(infrared_tim_tx.tx_timing_rest_duration > 0) {
            ...
            buffer->polarity[polarity_counter] =
                infrared_tim_tx.tx_timing_rest_level ? INFRARED_TX_CCMR_HIGH :
        ...
        if(num_of_impulses == 0) {
            ...
            buffer->polarity[polarity_counter] =
                level ? INFRARED_TX_CCMR_HIGH : INFRARED_TX_CCMR_LOW;
...
static void furi_hal_infrared_async_tx_free_resources(void) {
    ...
    if(infrared_tim_tx.stop_semaphore) {
...
void furi_hal_infrared_async_tx_start(uint32_t freq, float duty_cycle) {
    ...
    infrared_tim_tx.buffer[0].data = infrared_tx_data_0;
    infrared_tim_tx.buffer[1].data = infrared_tx_data_1;
    infrared_tim_tx.buffer[0].polarity = infrared_tx_polarity_0;
    infrared_tim_tx.buffer[1].polarity = infrared_tx_polarity_1;
...
void furi_hal_infrared_async_tx_wait_termination(void) {
    ...
    FuriStatus status =
        furi_semaphore_acquire(infrared_tim_tx.stop_semaphore, FuriWaitForever);
    furi_check(status == FuriStatusOk);
